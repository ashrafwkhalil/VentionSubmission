...
export class EnforceValidImportsApi extends Command {
  ...
  static usage = Command.Usage({
    ...
    examples: [["A basic example", "npm run ventionsubmission-cli enforce-valid-imports-api"]],
  })

  async execute(): Promise<number | void> {
    const invalidImportRegex = /import .*ventionsubmission\/[a-zA-Z]+\//gm
    ...
    if (fileContainingInvalidImports.length > 0) {
      const errorMessage = `${fileContainingInvalidImports.length} file(s) have invalid imports. They should NOT look like this: "@ventionsubmission/models/something/entity"`
